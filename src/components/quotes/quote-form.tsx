...
export type QuoteFormCustomer = { id: string; name: string };
export type QuoteFormProduct = {
  id: string;
  name: string;
  ...
  const {
    ...
    defaultValues: quote
      ? {
          ...
          notes: quote.notes,
          items: quote.items.map((item, index) => ({ ...item, id: String(index) })),
        }
      : {
          customerId: "",
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-6">
      <Card>
        ...
        <CardContent className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <Field>
            ...
            {errors.expiryDate && <FieldError>{errors.expiryDate.message}</FieldError>}
          </Field>
        </CardContent>
      </Card>
      ...
      <Card>
        ...
        <CardContent>
          <Textarea rows={3} placeholder="Terms, validity notes, etc." {...register("notes")} />
        </CardContent>
      </Card>
      ...
      <div className="flex justify-end gap-2">
        ...
        <Button type="submit" disabled={isSubmitting}>
          {isSubmitting ? "Saving…" : isEdit ? "Save changes" : "Create quote"}
        </Button>
      </div>
    </form>
  );
