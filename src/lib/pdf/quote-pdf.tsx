...
const STATUS_STYLES: Record<QuoteStatus, { bg: string; color: string; borderColor?: string }> = {
  DRAFT: { bg: "#eeeeee", color: "#4b4b4b" },
  SENT: { bg: "#ffffff", color: INK, borderColor: "#c9c9c9" },
  ACCEPTED: { bg: INK, color: "#ffffff" },
  REJECTED: { bg: DANGER, color: "#ffffff" },
  EXPIRED: { bg: DANGER, color: "#ffffff" },
  CONVERTED: { bg: "#eeeeee", color: "#4b4b4b" },
};
...
const styles = StyleSheet.create({
  ...
  quoteNumber: { fontSize: 20, fontWeight: 700, textAlign: "right", marginBottom: 8 },
  statusBadge: {
    alignSelf: "flex-end",
    borderRadius: 3,
    paddingVertical: 3,
  ...
  summaryTotalLabel: { fontSize: 11, fontWeight: 700, textTransform: "uppercase", letterSpacing: 0.4 },
  summaryTotalValue: { fontSize: 15, fontWeight: 700 },
    ...
    <Document title={data.quoteNumber}>
      <Page size="A4" style={styles.page}>
        ...
        <View style={styles.footer}>
          <Text style={styles.footerThanks}>Thank you for considering us.</Text>
          <Text style={styles.footerMeta}>
            {data.business.name} · Quote {data.quoteNumber}
          </Text>
        </View>
      </Page>
    </Document>
